...
Move *Player::doMove(Move *opponentsMove, int msLeft) {
    ...
    //Move *finalmove = Minimax(listMoves, this -> myside, this -> board);

    Move *finalmove = MiniMinimax(listMoves);

    this -> board -> doMove(finalmove, this -> myside);
...
Move *Player::Minimax(vector<Move*> listMoves, Side side, Board * board_)
{
	...
	for (unsigned int i = 0; i < listMoves.size(); i++)
	{
		...
		vector<Move*> theirMoves = this -> getPossibleMoves(other_side, copyBoard);
